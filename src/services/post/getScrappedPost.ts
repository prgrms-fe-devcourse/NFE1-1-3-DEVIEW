...
type GetScrappedPostResponseProps = Omit<CommonPostResponseProps, "posts">;

export async function getScrappedPost(): Promise<GetScrappedPostResponseProps> {
  try {
    const response = await axiosInstance.get<GetScrappedPostResponseProps>(`/post/scraps`, {
      headers: {
