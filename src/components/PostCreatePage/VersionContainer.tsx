...
export const VersionContainer = ({ state, onAddVersion, onRemoveVersion, onVersionChange }: VersionContainerProps) => {
  ...
  return (
    <section className="relative flex w-full flex-col gap-4 rounded-lg border border-solid border-gray px-4 py-6 shadow">
      <div className="flex flex-col gap-3">
        <h2 className="text-16 font-semibold md:text-20">언어와 버전을 입력해주세요</h2>
        <p className="text-14 text-primary md:text-16">답변자가 더 좋은 답변을 할 수 있게 정확히 기입해주세요.</p>
        <p className="text-14 text-primary md:text-16">첫번째 언어의 아이콘이 해당 게시글의 아이콘으로 지정됩니다.</p>
          ...
          {state.devDependencies.map((dependency, index) => (
            <div key={index} className="flex gap-[3%]">
              ...
              <input
                ...
                className="h-12 w-[77%] rounded-lg border-gray px-4 text-14 text-gray md:h-auto"
                type="text"
